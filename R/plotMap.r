...
#' Uses the \code{\link{map}} method of the maps package.
#'  
...
#' @param database character string naming a geographical database (see \code{\link{map}}).
#' @param regions character vector that names the polygons to draw (see \code{\link{map}}).
#' @param xlim the x limits (x1, x2) of the plot. 
#' @param ylim the y limits (y1, y2) of the plot.
#' @param xlab title of the x axis.
#' @param ylab title of the y axis.
#' @param map.default.text TRUE if default text is plotted using the \code{\link{map.text}} method.
...
#' @export
#' @seealso  \code{\link{plotGmap}}, \code{\link{map}}, \code{\link{border}}, \code{\link{plotBorder}}
#' @examples
#' data(kili)
#' plotMap(master)
#' plotMap(kili)
